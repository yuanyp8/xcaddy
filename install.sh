...
show_help() {
    ...
    echo ""
    echo -e "${BLUE}描述:${NC}"
    echo "  本脚本会自动将 Caddy 部署为类似 Nginx 的企业级架构。"
    echo "  自动创建 snippets(中间件层) 和 sites-available(业务层) 目录。"
    echo "  支持纯后端代理、以及前后端分离(静态托管+API转发)两种模式。"
    echo ""
    echo -e "${BLUE}必填参数 (可通过配置文件或命令行传入):${NC}"
    echo "  --ak <字符串>         阿里云 AccessKey ID (用于自动申请 HTTPS 证书)"
    echo "  --sk <字符串>         阿里云 AccessKey Secret"
    echo "  --domain <字符串>     要绑定的域名 (例如: api.example.com)"
    echo "  --backend <字符串>    后端服务地址 (例如: 127.0.0.1:8080)"
    echo ""
    echo -e "${BLUE}可选参数:${NC}"
    echo "  --frontend <绝对路径> 前端静态文件目录 (例如: /var/www/html/dist)"
    echo "                        不填: 纯反向代理模式"
    echo "                        填写: 启动前后端分离模式 (前端页面托管 + /api路径转发后端)"
...
# 核心安装逻辑
# ==========================================================
echo -e "${GREEN}[1/6] 停止旧服务并备份配置...${NC}"
...
echo -e "${GREEN}[2/6] 初始化系统与企业级目录环境...${NC}"
...
echo -e "${GREEN}[3/6] 部署二进制与系统服务...${NC}"
...
# ---------------------------------------------------------
# 生成企业级配置架构 (类似 Nginx 的 snippets/sites-enabled)
# ---------------------------------------------------------
echo -e "${GREEN}[4/6] 构建中间件层 与业务层...${NC}"
...
# ---------------------------------------------------------
echo ""
echo -e "${YELLOW}正在验证服务与证书状态 (首次申请需 10-30 秒)...${NC}"
