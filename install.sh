...
EOF
    # 3. 生成业务层站点配置 (纯网关模式)
    SITE_CONF="${SITES_AVAIL_DIR}/${LISTEN_ADDR}.conf"
    # 将域名中的 : 替换为 _ 作为安全的文件名 (如 harbor.example.com_9443.conf)
...
verify() {
    ...
    for i in {1..8}; do
        sleep 5
        # 动态使用监听端口进行 SSL 握手验证
        CERT_ISSUER=$(echo | openssl s_client -connect localhost:${LISTEN_PORT} -servername "${LISTEN_ADDR%%:*}" 2>/dev/null | openssl x509 -noout -issuer 2>/dev/null || true)
